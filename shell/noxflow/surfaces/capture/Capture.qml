...
// Stolen from: QuickSnip (dual-polarity OCR, word overlay, smart analysis, Lens upload).
...
PanelWindow {
    ...
    property var ocrWords: []     // [{x,y,w,h,text,confidence,line}]

    // Smart analysis result
    property string analysisType: ""  // "url", "code", "dictionary", "ai", "search"

    // ── Settings (created at root of shell, accessed globally) ──
    readonly property var settings: typeof shellRoot !== "undefined" && shellRoot.quickSnipSettings ? shellRoot.quickSnipSettings : null
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Rectangle { anchors.left: parent.left; anchors.top: parent.top; anchors.margins: -3; width: 6; height: 6; radius: 3; color: Theme.Tokens.tonalPrimary }
            Rectangle { anchors.right: parent.right; anchors.top: parent.top; anchors.margins: -3; width: 6; height: 6; radius: 3; color: Theme.Tokens.tonalPrimary }
            Rectangle { anchors.left: parent.left; anchors.bottom: parent.bottom; anchors.margins: -3; width: 6; height: 6; radius: 3; color: Theme.Tokens.tonalPrimary }
            Rectangle { anchors.right: parent.right; anchors.bottom: parent.bottom; anchors.margins: -3; width: 6; height: 6; radius: 3; color: Theme.Tokens.tonalPrimary }

            Text {
                anchors.bottom: parent.top; anchors.left: parent.left; anchors.margins: 4
    ...
    MouseArea {
        ...
        onReleased: function(mouse) {
            ...
            if (root.selW < 10 || root.selH < 10) { root.close(); return; }
    ...
    Rectangle {
        ...
        Behavior on scale { NumberAnimation { duration: Theme.Tokens.duration(150); easing.type: Easing.OutBack } }
        Behavior on opacity { NumberAnimation { duration: Theme.Tokens.duration(100) } }
        ...
        RowLayout {
            ...
            Components.IconButton { iconText: "📋"; accessibleName: "Copy to clipboard"; onClicked: root.performAction("copy") }
            Components.IconButton { iconText: "💾"; accessibleName: "Save screenshot"; onClicked: root.performAction("save") }
            Components.IconButton { iconText: "🔤"; accessibleName: "OCR (text recognition)"; onClicked: root.performOcr() }
            Components.IconButton { iconText: "R";  accessibleName: "Raw OCR layout"; checked: root.settings ? root.settings.rawMode : false; onClicked: { if (root.settings) root.settings.rawMode = !root.settings.rawMode } }
            Components.IconButton { iconText: "S";  accessibleName: "Single line"; checked: root.settings ? root.settings.singleLine : false; onClicked: { if (root.settings) root.settings.singleLine = !root.settings.singleLine } }
            Components.IconButton { iconText: "D";  accessibleName: "Direct mode (copy image)"; checked: root.settings ? root.settings.directMode : false; onClicked: { if (root.settings) { root.settings.directMode = !root.settings.directMode; if (root.settings.directMode) root.performAction("copy"); } } }
            Components.IconButton { iconText: "🌐"; accessibleName: "Translate via Lens"; onClicked: root.performAction("lens") }
            Components.IconButton { iconText: "🔍"; accessibleName: "Search image"; onClicked: root.performAction("search") }
            Components.IconButton { iconText: "✕"; accessibleName: "Close"; onClicked: root.close() }
        }
    }

    // ── OCR result overlay (text view) ──
    Rectangle {
        anchors.centerIn: parent
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Text { text: "OCR Result"; color: Theme.Tokens.textPrimary; font.pixelSize: Theme.Tokens.typographyTitleLarge; font.bold: true; Layout.fillWidth: true }
                ...
                Components.IconButton { iconText: "📋"; accessibleName: "Copy OCR text"; onClicked: root.copyText(root.ocrText) }
                ...
                Components.IconButton {
                    ...
                    onClicked: { root.showOcrResult = false; root.showWordOverlay = true; }
                }
            }
            ...
            RowLayout {
                Layout.fillWidth: true
                visible: root.ocrWords.length > 0
                Components.Toggle {
    ...
    // ── Word overlay (over selection region) ──
    WordOverlay {
        id: wordOverlay
        ...
        regionY: root.selY
        scaleFactor: 1.0
        onWordCopied: function(text) {
            root.copyText(text);
        }
        onWordSelected: function(index, text) {
            // Show the result panel with this word highlighted
            root.ocrText = text;
            root.showOcrResult = true;
        }
    }
    ...
    function performAction(action) {
        ...
        var geom = Math.round(root.selW) + "x" + Math.round(root.selH) + "+" +
                   Math.round(root.selX) + "+" + Math.round(root.selY);
        ...
        switch (action) {
            case "copy":
                ...
                break;
            case "save":
                saveScreenshot.command = ["sh", "-c", "grim -g \"" + region + "\" \"$HOME/Pictures/Screenshots/capture-$(date +%s).png\""];
                saveScreenshot.running = true;
                root.close();
                break;
            case "lens":
                // Upload via curl to Google Lens, open result
    ...
    property Process searchCapture: Process {
        ...
        onExited: function(code, status) {
            if (code !== 0) return;
            openUrlProcess.command = ["xdg-open", "https://images.google.com/searchbyimage?image_url=file://" + searchImageFile];
    ...
    // ── Copy to clipboard (via Process stdin) ──
    function copyText(text) {
        copyToClipboard.command = ["wl-copy"];
        copyToClipboard.running = true;
    }
    ...
    property Process copyToClipboard: Process {
        ...
        stdinEnabled: true
        onStarted: {
            copyToClipboard.write(root.ocrText);
            copyToClipboard.stdinEnabled = false;
        }
    }

    property Process notifyProcess: Process { running: false }

    property Process copySelection: Process {
        running: false
        onStarted: {
            copySelection.write(root.ocrText);
            copySelection.stdinEnabled = false;
        }
        stdinEnabled: true
    }
    ...
    // ── Dual-polarity OCR with TSV parsing ──
    function performOcr() {
        var region = Math.round(root.selX) + "," + Math.round(root.selY) + " " +
                     Math.round(root.selW) + "x" + Math.round(root.selH);
        ...
        root.showOcrResult = true;
        root.ocrText = "Recognizing text…";
        root.ocrWords = [];
        root.analysisType = "";

        // Build adaptive OCR command from QuickSnip patterns
        var w = Math.round(root.selW);
        var h = Math.round(root.selH);
        var area = w * h;
        var isSmall = area < 40000;
        var isTiny = h < 30;
        var upscale = "";
        if (isTiny) upscale = "-resize 400%";
        else if (isSmall) upscale = "-resize 200%";
        var psm = root.settings && root.settings.rawMode ? "6" : (isTiny || h < 50 || w / h > 10) ? "7" : "3";

        // Post-processing: rawMode preserves layout, singleLine flattens
        var postProcess = "";
        if (root.settings && root.settings.rawMode) {
            postProcess += " | sed 's/[[:space:]]*$//'";
        } else if (root.settings && root.settings.singleLine) {
            postProcess += " | awk 'BEGIN{RS=\"\"; ORS=\"\\n\\n\"} {$1=$1; print}' | sed 's/[[:space:]]*$//' | tr '\\n' ' ' | sed 's/  */ /g; s/[[:space:]]*$//'";
        } else {
            postProcess += " | awk 'BEGIN{RS=\"\"; ORS=\"\\n\\n\"} {$1=$1; print}' | sed 's/[[:space:]]*$//'";
        }

        ocrProcess.command = ["sh", "-c",
            "grim -g \"" + region + "\" -s 2 - | magick - " + upscale + " -colorspace Gray -depth 8 - | tesseract stdin stdout -l " +
            root.ocrLanguage + " --oem 1 --psm " + psm + " 2>/dev/null" + postProcess];
        ocrBuffer = "";
        ocrProcess.running = true;
    }

    property string ocrBuffer: ""
    property string ocrTsvBuffer: ""
    ...
    property Process ocrProcess: Process {
        ...
        onExited: function(code, status) {
            ...
            if (code !== 0 || text === "") {
                root.ocrText = "OCR failed. Is tesseract installed?";
                return;
            }

            root.ocrText = text;
            root.analysisType = root.analyzeText(text);

            // Notify with first 100 chars
            var clean = text.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
            var snippet = clean.substring(0, 100);
            if (clean.length > 100) snippet += "…";
            notifyProcess.command = ["sh", "-c", "notify-send 'OCR Copied' '" + snippet.replace(/'/g, "'\\''") + "' -t 3000"];
            notifyProcess.running = true;

            // Now run TSV pass for word overlay
            var region = Math.round(root.selX) + "," + Math.round(root.selY) + " " +
                         Math.round(root.selW) + "x" + Math.round(root.selH);
            tsvProcess.command = ["sh", "-c",
                "grim -g \"" + region + "\" -s 2 - | tesseract stdin stdout -l " +
                root.ocrLanguage + " tsv 2>/dev/null"];
            tsvBuffer = "";
            tsvProcess.running = true;
        }
    }
    ...
    function parseTsv(tsv) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (conf < 10) continue;  // skip low-confidence
            ...
            words.push({
                ...
                text: parts[colText] || "",
                confidence: conf,
                line: 0, // would need line_num from TSV; skip for now
            });
        }
    ...
    function analyzeText(text) {
        ...
        // Code detection (>3 lines with code patterns)
        var codeCount = 0;
        var lines = text.split("\n");
        for (var i = 0; i < lines.length; i++) {
            var line = lines[i].trim();
            if (/^(function|const|let|var|if|for|while|import|export|def |class |int |String|public |private |=>)/.test(line)) codeCount++;
            if (/[{}().;]$/.test(line)) codeCount++;
            if (/^[a-zA-Z_]\w*\s*\(/.test(line)) codeCount++;
        }
        if (codeCount >= 3 || (codeCount >= 2 && lines.length >= 3)) return "code";

        // Dictionary (single short word)
        if (lines.length === 1 && text.trim().split(/\s+/).length <= 2 && text.length < 60) return "dictionary";

        // AI (many words — likely needs explanation)
    ...
    function openAnalyzedText() {
        if (root.analysisType === "url") {
            ...
            // Send to launcher's Ask AI mode via IPC
    ...
    SequentialAnimation {
        ...
        onFinished: root.captureOpen = false
    }

    function toggle() {
        if (captureOpen) close(); else open();
    }
}
