...
PanelWindow {
    ...
    Rectangle {
        ...
        border.width: 1
        scale: 0.85 + 0.15 * (lifecycle.openProgress)
        ...
        ColumnLayout {
            ...
            Item { Layout.fillWidth: true; Layout.fillHeight: true; clip: true
                ...
                Text { anchors.centerIn: parent; text: root.searchText === "" ? "Type to search..." : "No results"; color: Theme.Tokens.textMuted; font.pixelSize: Theme.Tokens.typographyBodyMedium; visible: root.filteredResults.length === 0 && !(root.activeMode === 4 && (root.aiResponse !== "" || root.aiLoading)) }
                ListView { id: resultsList; anchors.fill: parent; model: root.filteredResults; currentIndex: root.selectedIndex; spacing: Theme.Tokens.spacingXs; visible: !(root.activeMode === 4 && (root.aiResponse !== "" || root.aiLoading))
                    delegate: Rectangle {
                        ...
                        width: parent ? parent.width : 0; height: Theme.Tokens.scaled(Theme.Tokens.heightControl); radius: Theme.Tokens.radiusSm
                        ...
                        RowLayout { anchors.fill: parent; anchors.margins: Theme.Tokens.spacingSm; spacing: Theme.Tokens.spacingMd
                            Text { text: modelData.icon || "◆"; color: index === root.selectedIndex ? Theme.Tokens.tonalOnPrimaryContainer : Theme.Tokens.tonalPrimary; font.pixelSize: Theme.Tokens.iconMd }
                            ColumnLayout { Layout.fillWidth: true; spacing: 0
                                Text { text: modelData.title || modelData.name || "Unknown"; color: index === root.selectedIndex ? Theme.Tokens.tonalOnPrimaryContainer : Theme.Tokens.textPrimary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyBodyMedium; elide: Text.ElideRight; Layout.fillWidth: true }
                                Text { text: modelData.subtitle || modelData.description || modelData.comment || ""; color: index === root.selectedIndex ? Theme.Tokens.tonalOnPrimaryContainer : Theme.Tokens.textSecondary; font.pixelSize: Theme.Tokens.typographyLabelSmall; elide: Text.ElideRight; Layout.fillWidth: true; visible: text !== "" }
                            }
                            Text { text: modelData.shortcut || ""; color: index === root.selectedIndex ? Theme.Tokens.tonalOnPrimaryContainer : Theme.Tokens.textMuted; font.pixelSize: Theme.Tokens.typographyLabelSmall; visible: text !== "" }
                        }
                        TapHandler { onTapped: { root.selectedIndex = index; root.activateSelected(); } }
                        HoverHandler { cursorShape: Qt.PointingHandCursor }
                    }
                }
            }
    ...
    function executeAiQuery(query) {
        aiLoading = true; aiStatus = 1; aiResponse = "";
        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.status === 200) {
                ...
                catch (e) { aiResponse = "Failed to parse AI response"; aiStatus = 3; }
            } else { aiResponse = "AI request failed (HTTP " + xhr.status + ")"; aiStatus = 3; }
        };
    ...
    function resetAi() { aiQuery = ""; aiResponse = ""; aiLoading = false; aiStatus = 0; if (aiXhr) { aiXhr.abort(); aiXhr = null; } }
    ...
    property Process desktopScanner: Process { id: desktopScanner; running: false; stdout: SplitParser { splitMarker: ""; onRead: function(data) { root.desktopBuffer += data; } }
        onExited: function(code, status) { if (root.desktopBuffer) { root.parseDesktopFiles(root.desktopBuffer); root.desktopBuffer = ""; root.filterResults(); } }
    }
    ...
    function parseDesktopFiles(data) {
        ...
        var iconMap = { "firefox":"🦊", "firefox-esr":"🦊", "librewolf":"🦊", "kitty":"⚡", "alacritty":"⚡", "wezterm":"⚡", "foot":"⚡", "thunar":"📁", "nautilus":"📁", "dolphin":"📁", "pcmanfm":"📁", "code":"💻", "codium":"💻", "code-oss":"💻", "obsidian":"📝", "logseq":"📝", "discord":"💬", "vesktop":"💬", "telegram":"💬", "element":"💬", "spotify":"🎵", "ncmpcpp":"🎵", "mpd":"🎵", "gimp":"🎨", "inkscape":"🎨", "krita":"🎨", "blender":"🔄", "godot":"🎮", "libreoffice":"📄", "onlyoffice":"📄", "evince":"📖", "zathura":"📖", "okular":"📖", "keepassxc":"🔑", "bitwarden":"🔑", "virt-manager":"🖥️", "gnome-boxes":"🖥️", "steam":"🎮", "lutris":"🎮", "heroic":"🎮", "gparted":"💽", "gnome-disks":"💽" };
        for (var i = 0; i < lines.length; i++) { var parts = lines[i].split("|"); if (parts.length < 2) continue; var title = parts[0].trim(); var cmd = parts[1].trim(); var iconName = parts.length > 2 ? parts[2].trim().toLowerCase() : ""; var basename = cmd.split("/").pop().split(" ")[0].toLowerCase(); var icon = iconMap[basename] || iconMap[cmd] || iconMap[iconName] || "◆"; if (!title || !cmd) continue; apps.push({ title: title, icon: icon, subtitle: cmd, action: "launch", actionParams: { command: cmd } }); }
        appCache = apps.length > 0 ? apps : buildDefaultApps();
    }

    function buildDefaultApps() { return [{ title:"Firefox", icon:"🦊", subtitle:"Web browser", action:"launch", actionParams:{command:"firefox"}}, { title:"Kitty", icon:"⚡", subtitle:"Terminal emulator", action:"launch", actionParams:{command:"kitty"}}, { title:"Thunar", icon:"📁", subtitle:"File manager", action:"launch", actionParams:{command:"thunar"}}, { title:"Code", icon:"💻", subtitle:"Visual Studio Code", action:"launch", actionParams:{command:"code"}}, { title:"Settings", icon:"☰", subtitle:"System settings", action:"launch", actionParams:{command:"gnome-controlcenter"}}, { title:"Obsidian", icon:"📝", subtitle:"Notes", action:"launch", actionParams:{command:"obsidian"}}]; }
    ...
    function buildCommandResults() { return [{ title:"Lock", icon:"🔒", subtitle:"Lock the screen", action:"lock", actionParams:{} }, { title:"Suspend", icon:"⏾", subtitle:"Suspend to RAM", action:"suspend", actionParams:{} }, { title:"Reboot", icon:"↻", subtitle:"Reboot the system", action:"reboot", actionParams:{} }, { title:"Power Off", icon:"⏻", subtitle:"Shut down", action:"power_off", actionParams:{} }, { title:"Toggle DND", icon:"⊘", subtitle:"Do not disturb", action:"toggle_dnd", actionParams:{} }, { title:"Screenshot", icon:"◉", subtitle:"Take a screenshot", action:"screenshot", actionParams:{} }, { title:"Reload shell", icon:"↻", subtitle:"Reload NoxFlow shell", action:"reload_shell", actionParams:{} }]; }

    function evaluateCalc(expr) { try { var sanitized = expr.replace(/[^0-9+\-*/.()% ]/g, ""); var result = new Function("return (" + sanitized + ")")(); if (typeof result === "number" && isFinite(result)) return String(Math.round(result * 100) / 100); return "?"; } catch (e) { return "?"; } }
