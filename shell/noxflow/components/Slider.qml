...
FocusScope {
    ...
    MouseArea { anchors.fill: parent; onPressed: root.setFromPosition(mouse.x); onPositionChanged: if (pressed) root.setFromPosition(mouse.x) }
    Keys.onLeftPressed: root.setFromPosition(root.width * ((root.value - root.from) / (root.to - root.from)) - 8)
    Keys.onRightPressed: root.setFromPosition(root.width * ((root.value - root.from) / (root.to - root.from)) + 8)
    function setFromPosition(position) { var next = root.from + (Math.max(0, Math.min(root.width, position)) / root.width) * (root.to - root.from); root.value = next; root.moved(next) }
}
