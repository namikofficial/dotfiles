    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    Layout.fillWidth: true
                    Text { text: "CPU  " + (root.systemModel.ready ? Math.round(root.systemModel.cpuUsage) + "%" : "—"); color: Theme.Tokens.textPrimary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyBodySmall; font.bold: true }
                    Item { Layout.fillWidth: true }
                    Text { text: root.systemModel.cpuFreq > 0 ? (root.systemModel.cpuFreq / 1000).toFixed(1) + " GHz" : "Frequency unavailable"; color: Theme.Tokens.textSecondary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyLabelSmall }
                }
                Text { text: "Load  " + root.systemModel.load1.toFixed(2) + " · " + root.systemModel.load5.toFixed(2) + " · " + root.systemModel.load15.toFixed(2); color: Theme.Tokens.textSecondary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyLabelSmall }
                Text { text: "Memory  " + (root.systemModel.memUsed / 1048576).toFixed(1) + " / " + (root.systemModel.memTotal / 1048576).toFixed(1) + " GiB" + (root.systemModel.swapTotal > 0 ? "  ·  Swap " + (root.systemModel.swapUsed / 1048576).toFixed(1) + " GiB" : ""); color: Theme.Tokens.textPrimary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyBodySmall }
                Text { text: "Thermals  CPU " + (root.systemModel.cpuTemp > 0 ? Math.round(root.systemModel.cpuTemp) + "°C" : "—") + "  ·  GPU " + (root.systemModel.gpuTemp > 0 ? Math.round(root.systemModel.gpuTemp) + "°C" : "—"); color: root.systemModel.cpuTemp >= 85 ? Theme.Tokens.stateDanger : root.systemModel.cpuTemp >= 75 ? Theme.Tokens.stateWarning : Theme.Tokens.textSecondary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyBodySmall }
                Text { visible: root.systemModel.gpuAvailable; text: "GPU  " + Math.round(root.systemModel.gpuUsage) + "%  ·  VRAM " + Math.round(root.systemModel.gpuMemUsed) + " / " + Math.round(root.systemModel.gpuMemTotal) + " MiB" + (root.systemModel.gpuPower > 0 ? "  ·  " + root.systemModel.gpuPower.toFixed(1) + " W" : ""); color: Theme.Tokens.textSecondary; font.family: Theme.Tokens.typographyFontFamily; font.pixelSize: Theme.Tokens.typographyLabelSmall }
