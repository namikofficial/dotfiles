...
case "$choice" in
  ...
  "create note from clipboard")
    exec kitty --title "clipboard note" -e /usr/bin/zsh -lic 'text="$(wl-paste -n 2>/dev/null || true)"; file="$HOME/Documents/notes/clipboard-$(date +%Y%m%d-%H%M%S).md"; printf "# Clipboard Note\n\n%s\n" "$text" > "$file"; command -v code >/dev/null 2>&1 && code "$file" >/dev/null 2>&1 &'
    ;;
esac
